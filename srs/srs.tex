...
\includegraphics[width=\textwidth]{diagram/environnements.png}
Le but premier de Nitcorn est d'offrir aux applications développées en Nit la possibilité
d'utiliser un serveur web. On va donc devoir créer un module pour le langage Nit afin que d'autres applications Nit puissent être en mesure d'utiliser un serveur web. Il offrira des fonctionnalités de serveur au langage afin qu'il puisse être placé derrière un serveur web(Nginx, Apache,\ldots). Sous cette configuration, il va pouvoir répondre à des dizaines de milliers de requêtes web par seconde. Un service web sera utilisé afin de configurer le serveur. La gestion des logs ou la persistance de la configuration devront être gérées automatiquement par le serveur web.\\ De cette façon, l'utilisateur pourra avoir accès au log pour déterminer l'état du serveur. De plus, lorsqu'une erreur est détectée, un message d'erreur sera retournée à l'utilisateur. Nitcorn sera directement exposée à Internet et devra agir en tant que serveur web exactement comme c'est le cas pour les autres
...
communiquer avec le web sans avoir à se soucier des configurations. Cette
...
Nitcorn pour répondre à des requêtes web dynamiquement. De plus, une API est existante pour la gestion de la configuration selon les préférences de l'utilisateur.\\
\begin{figure}[ht!]
...
\textbf{file descriptors} ouverts en même temps sera limitée par le noyau du
...
La spécification complète et exacte des méthodes, des \textit{headers} et des codes de status suivants se retrouvent dans le RFC2616 listé dans les références.
...
\begin{description}
 ...
 \item [GET] Demande la ressource identifiée par l'URI.
 \item [HEAD] Identique à GET, mais la réponse ne doit pas contenir de \textit{message body}.
 \item [POST] Indique que des données sont envoyées dans le \textit{message body} et demande au serveur d'en prendre compte.
 \item [PUT] Demande de stocker les données envoyées par la requête.
...
\subsubsection{Request-Headers:}
        ...
        \item [If-Modified-Since] La requête doit avoir été modifiée depuis la date spécifiée.
        \item [Referer] URL du lien de la requête effectuée.          
        \item [User-Agent] Liste d'informations sur le client(nom et version)       
        \end{description}
...
\subsubsection{Entity-Headers:}
        \begin{description}
        \item [Allow] Liste de méthodes supportées par la ressource                
...
\subsubsection{General-Headers:}
        \begin{description}
        \item [Cache-Control] Directives imposées aux systèmes de cache rencontrés dans la requête et la réponse.      
        \item [Connection] Options des spécifications de la connection courante.     
        \item [Trailer] Liste des headers présents dans le message encodé.          
        \item [Transfer-Encoding] Indique le type de transformation effectuée sur le corps du message afin de se rendre à destination.
        \item [Upgrade] Spécifie les autres protocoles de communication supportés au serveur.          
        ...
        \item [Warning] Contient des informations additionnelles sur le status ou la transformation du message.
        \end{description}
\subsubsection{Request-Headers:}
        \begin{description}
        ...
        \item [Host] Spécifie l'hôte et le port de la ressource demandée.
        \item [If-Match] Rend conditionnelle la requête.
        \item [If-None-Match] Semblable au \textit{If-Match}, mais inverse la condition.
        \item [If-Range] Spécifie de n'envoyer que les parties à jour de la ressource demandée dont le client ne dispose pas.
        \item [If-Unmodified-Since] Rend conditionnelle la méthode: n'effectue la requête que si la ressource n'a pas été modifiée depuis la date indiquée.
        \item [Max-Forwards] Limite le nombre de proxys ou de passerellee qui peuvent envoyer une réponse lors d'une requête TRACE ou OPTIONS.
        \item [Proxy-Authorization] Permet au client de s'identifier à un proxy.
        \item [Range] Spécifie l'étendu de la ressource désirée par le client.
        \item [TE] Liste des extentions d'encodage de transfert. 
        \end{description}
\subsubsection{Response-Headers:}
        \begin{description}
        \item [Accept-Ranges] Permet au serveur d'inquiquer les \textit{Range} acceptés au client.
...
\subsubsection{Entity-Headers:}
        ...
        \item [Content-Location] Indique un emplacement alternatif du contenu.
...
\subsubsection{Le serveur doit retourner ces codes de status HTTP/1.0 et HTTP/1.1 résumés brièvement ici.}
 \begin{description}
 \item [200 - OK] La requête à bien été traitée sans erreurs.
 ...
 \item [204 - No Content] Le serveur a bien traité la requête et il n'y a aucune information à retourner.
 \item [205 - Reset Content] Le serveur a complété la requête et le client devrait rafraîchir le document qui a causé l'envoie de la requête.
 \item [206 - Partial Content] Le serveur a complété une requête GET partielle et retourne le \textit{Range} demandé de la ressource.
 \item [301 - Moved Permanently] La ressource demandée à été assignée à un nouvel URL de façon permanante.
 ...
 \item [406 - Not Acceptable] La ressource identifiée par la requête est seulement capable de générer des entitées de réponses qui ont des caractéristiques de contenu non acceptable selon les \textit{headers} d'acceptation envoyés par la requête.
 ...
 \item [409 - Conflict] La requête n'a pas pu être complétée à cause d'un conflit avec l'état actuel de la ressource.
 ...
 \item [412 - Precondition Failed] La précondition donnée dans un ou plusieurs champs \textit{header} a été évaluée à faux lorsque testée sur le serveur.
 ...
 \item [414 - Request-URI Too Long] Le serveur refuse de servir la requête car l'URI de la requête est plus long que ce que le serveur accepte d'interpréter.
 ...
 \item [505 - HTTP Version Not Supported] Le serveur ne supporte pas, ou refuse de supporter, la version du protocole HTTP spécifiée par la requête.
 \end{description}
...
Étant donné le sujet de l'application, il est essentiel d'avoir une connexion réseau (Internet ou LAN) pour avoir accès au serveur web. De plus, le serveur Nitcorn est directement dépendant de l'architecture compatible avec le compilateur Nitc.
...
\subsection{Administrateur:} L'administrateur est la personne qui s'occupe de la configuration et de la maintenance d'une des applications installées sur le serveur, par exemple un site web.
...
Les cas d'utilisation ont une priorité qui leur est assignée. 1 étant les plus prioritaires et plus le nombre associé est croissant, moins le cas est prioritaire.
...
\includegraphics[width=\textwidth]{./diagram/infrastructure.png}	
...
\begin{itemize}
    ...
    \item Le host
\end{itemize}
Une requête doit être valide sinon on retourne un message d'erreur. Avec les informations que j'ai, je vais pouvoir lire ma configuration actuelle. Si
...
\\
Le premier élément de la configuration que je lis pour ce host est pour déterminer
...
\subsubsection{Postconditions:}
\begin{itemize}
    \item On va effectuer une recherche dans la base de donnée pour récupérer la configuration la plus récente.
    \item L'événement est enregistré dans le fichier log.
    \item La configuration est changée.
\end{itemize} 

\subsubsection{Description du processus:} En tant que serveur, je vais faire une première lecture de mes configurations dans la base de données afin de connaître comment traiter les requêtes que je vais recevoir.
...
\subsubsection{Postconditions:}
\begin{itemize}
    \item  Le serveur est informé que la configuration a changée.
    \item  L'événement est enregistré dans le fichier log.
\end{itemize} 
...
je dois ajuster mes attributs de classe et valeurs dans la base de données
...
\subsubsection{Préconditions:}
\begin{itemize}
    \item  Un fichier Log doit être existant sinon on le créer.
\end{itemize} 
\subsubsection{Postconditions:}
\begin{itemize}
...
\end{itemize} 
\subsubsection{Description du processus:}En tant qu'administrateur, si je veux savoir l'état d'un ou plusieurs paramètre(s) de
...
du ou des paramètre(s) désirée(s) à l'API qui se chargera de relayer l'information à la classe appropriée.
...
\subsubsection{Rapport avec les autres intrants/extrants} La configuration détermine la façon dont la requête sera traitée par le serveur.

\section{Exigences fonctionnelles}
\begin{itemize}
    \item Lors du démarrage, le système doit lire sa configuration enregistrée dans la base de données.
...
\end{itemize}
\section{Exigences de performance} Le serveur sera en mesure de gérer plus de 10 000 requêtes simultanées qui devront être traitées en moins de 100 millisecondes chacune. 
...
Le serveur Nitcorn aura besoin des données de cette base de données pour fonctionner, donc en contrepartie d'une petite charge, l'intégrité des données est primordiale. Elle ne devra donc être victime d'aucune perte de données dans la mesure du possible et devra offrir la possibilité de copier son contenu pour une sauvegarde de sûreté et d'importer une telle sauvegarde.
...
Le serveur web doit être disponible 99\% du temps entre le démarrage du serveur et jusqu'à ce que l'administrateur système le ferme.
\subsection{Sécurité}
Seul un utilisateur autorisé (i.e.: administrateur système) peut modifier la configuration du serveur. De plus, le serveur
ne doit pas poser de problème de sécurité pour le système d'exploitation.
\subsection{Maintenabilité}
Le serveur devra pouvoir changer sa configuration sans être redémarré.
