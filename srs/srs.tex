...
=======
 ...
 \subitem Le serveur est incapable de contacter une autre ressource nécessaire à la completion de la requête.
 \item \textbf{503 Service Unavailable}
 \subitem Le serveur manque temporairement de ressources pour répondre à la requête.
 \item \textbf{504 - Gateway Time-Out}
 \item \textbf{505 - HTTP Version not supported}
>>>>>>> 3fb99c1e478ac282b51863f655622b26d6b21f09
...
\begin{itemize}
    ...
    \item Lors d'une modification de la configuration effectuée à l'aide du service web, le système doit tenir compte des modifications et les sauvegarder.
\end{itemize}
\section{Exigences logiques relatives aux bases de données} Le serveur Nitcorn communique avec une base de données afin d'y stocker les informations 
de sa configuration. La fréquence d'utilisation des données est relative à la fréquence des redémarrages du serveur ainsi que les modifications faites à
sa configuration via le service web. Il y aura un compte administrateur sur la base de données que le service web utilisera pour modifier les données de configuration.
...
\section{Contraintes de conception} Le logiciel doit respecter plusieurs contraintes de conception vu qu'il doit envoyer des réponses respectant le 
protocol HTTP. Le logiciel doit respecter tous les normes ayant comme sujet l'envoi des réponses HTTP comme le RFC2616.
\section{Attributs}
