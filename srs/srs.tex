...
\section{Références}
(@TODO AJOUTER LES DATES ET LES ORGANISATIONS QUI LES ONT PUBLIÉS)\\
...
d'utiliser un serveur web. On va donc devoir créer un module pour le langage Nit afin que d'autres applications Nit puissent être en mesure d'utiliser un serveur web. Il offrira des fonctionnalités de serveur au langage afin qu'il puisse être placé derrière un serveur web(Nginx, Apache,\ldots). (@TODO: TROP VAGUE: )Sous cette configuration, il va pouvoir répondre à des dizaines de milliers de requêtes web. Un service web sera utilisé afin de configurer le serveur. La gestion des logs ou la persistance des configuration devront être gérés automatiquement par le serveur web.\\ De cette façon, l'utilisateur pourrait avoir accès au log pour déterminer l'état du serveur. De plus, lorsqu'une erreur est détectée, un message d'erreur sera retourné à l'utilisateur. Nitcorn sera directement exposé à Internet et devra agir en tant qu'un serveur web exactement comme c'est le cas pour les autres
...
(@TODO MESSAGE SUR LE FAIT QUE LA DÉFINITION EXACTE DES METHODES, HEADERS ET CODE D'ERREURS SONT DANS LE RFC 2616)\\
Le serveur devra supporter les méthodes HTTP/1.0 et HTTP/1.1 suivantes:\\
(@TODO BRÈVE DESCRIPTION DES MÉTHODES)\\
\begin{itemize}
 \item OPTIONS:
 \item GET:
 \item HEAD:
...
(@TODO COMPLÉTER LES DESCRIPTIONS MANQUANTES)\\
\\
...
Http/1.1\\
    General-Header\\
        ...
        \item Upgrade: Spécifie les autres prtocoles de communication supportées au server.          
        \item Via:              
        \item Warning:          
        \end{itemize}
    Request-Header\\
...
(@TODO MESSAGE SUR LA PORTABILITÉ SUR LES AUTRES OS QUI SERAIENT SUPPORTÉS PAR NIT?)\\
...
Je dois pouvoir recevoir une ou plusieurs requêtes simultanément(@TODO: COMBIEN PLUS PRÉCISÉMENT: MAXIMUM). Après l'envoie d'une requête effectuée par le client, je vais écouter un ou plusieurs port(s) à l'aide d'un socket. Ensuite, je vais identifier le type de requête qui m'a été transmis et l'assigner au module de traitement sous forme de texte.
...
(@TODO: 5.3.4 Logical database requirements
