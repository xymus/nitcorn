...
d'abstraction sera développée pour utiliser les librairies externes avec l'interface native de
Nit. 
\begin{itemize}
...
librairies déjà existantes matures et testées en production permettant de répondre à ces demandes.
...
permettre d'utiliser qu'un seul processus pour le serveur, tout en restant très performant.
...
\subsection{Administrateur système:} Le sysadmin est la personne qui s'occupe de la configuration et de la maintenance du serveur et du système sur lequel est installé le serveur. C'est la personne possédant le plus de droits sur le serveur.
\subsection{Administrateur:} Un adminisrateur est la personne qui s'occupe de la configuration et de la maintenant d'une des applications installées sur le serveur, par exemple un site web.
